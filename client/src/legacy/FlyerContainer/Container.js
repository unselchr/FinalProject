...
class FlyerContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      units: props.units||[],
  ...
  updateNums() {
    if (this.state.units) {
      if (this.state.units.length) {
        ...
        this.state.returnNums({category:"flyer",points:pt,powerLevel:pl});
      }
      else {
        this.setState({ totalPoints: this.state.units.points, totalPL: this.state.units.powerLevel });
        this.state.returnNums({ category: "flyer", points: this.state.units.points, powerLevel: this.state.units.powerLevel });
      }
    }
    ...
    return (
      <div>
        ...
        <ReactModal ariaHideApp={false} onRequestClose={this.handleCloseNew} isOpen={this.state.showNewModal} contentLabel="New unit">
          <NewUnitModal options={units.flyer} next={this.addHQNext} clickHandler={this.addHQ} />
