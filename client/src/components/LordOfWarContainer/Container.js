...
class LordOfWarContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      units: props.units||[],
  ...
  updateNums() {
    if (this.state.units) {
      if (this.state.units.length) {
        ...
        this.state.returnNums({category:"lordOfWar",points:pt,powerLevel:pl});
      }
      else {
        this.setState({ totalPoints: this.state.units.points, totalPL: this.state.units.powerLevel });
        this.state.returnNums({ category: "lordOfWar", points: this.state.units.points, powerLevel: this.state.units.powerLevel });
      }
    }
    ...
    return (
      <div>
        ...
        <ReactModal ariaHideApp={false} onRequestClose={this.handleCloseNew} isOpen={this.state.showNewModal} contentLabel="New unit">
          <NewUnitModal options={units.lordOfWar} next={this.addHQNext} clickHandler={this.addHQ} />
          <button onClick={this.handleCloseNew}>Close</button>
        </ReactModal>
        <div className="card" onClick={this.handleOpenNew}>
          <div className="card-body">
            <button>
              <h5 className="card-title">&#43;Lord Of War</h5>
            </button>
          </div>
        </div>
      </div>
...
export default LordOfWarContainer;